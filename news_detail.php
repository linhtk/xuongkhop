<? 
	...
	if($row['news_image'])
	{
		if(file_exists("images/news/thumb_0_".$row['news_image']))
		{
			$row['news_image'] = "<img src='images/news/thumb_0_".$row['news_image']."' />";
		} else 
		{
			$row['news_image'] = "";
		}
	...
	$row['item_date'] = formatDateFromDatabase($row['item_date']);
	$xtpl->assign("row",$row);
	//other
	$sql1 = "SELECT md5(news_id) AS news_id, news_title FROM tg_news WHERE news_id <> '".$row['news_id']."' AND news_active = '1' LIMIT 0,8";
	$rs1 = execSQL($sql1);
