...
if ($action == "Action") {
    if (strpos($edit_id, "~~~") === false) {
        ...
        $sql = "DELETE FROM " . TABLE_PREFIX . "test_question WHERE md5(id)='" . $edit_id . "'";
        execSQL($sql);
        $sql2 = "DELETE FROM " . TABLE_PREFIX . "test_answer WHERE q_id='" . $edit_id . "'";
        execSQL($sql2);
    } else {
        ...
        if (is_array($arr_del)) {
            for ($i = 0; $i < count($arr_del); $i++) {
                ...
                $sql = "DELETE FROM " . TABLE_PREFIX . "share WHERE md5(id)='" . $edit_id . "'";
                execSQL($sql);
                $sql2 = "DELETE FROM " . TABLE_PREFIX . "test_answer WHERE q_id='" . $edit_id . "'";
                execSQL($sql2);
            }
