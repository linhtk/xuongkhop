<?php
	...
	if($action=="Action")
	{
		if(strpos($edit_id,"~~~")===false)
		{
			...
			$sql="SELECT adv_image FROM ".TABLE_PREFIX."adv WHERE md5(adv_id)='$edit_id'";
			$rs=recordset($sql);
			$adv_image=$rs['adv_image'];
			deletefile("../images/adv/thum_0_".$adv_image);
			$sql="DELETE FROM ".TABLE_PREFIX."adv WHERE md5(adv_id)='".$edit_id."'";
			execSQL($sql);
