<?php
	...
	if($action=="Action")
	{
		if(strpos($edit_id,"~~~")===false)
		{
			...
			$sql="SELECT hoacuoi_anh FROM ".TABLE_PREFIX."hoacuoi WHERE md5(hoacuoi_id)='$edit_id'";
			$rs=recordset($sql);
			$hoacuoi_anh=$rs['hoacuoi_anh'];
			deletefile("../upload/hoacuoi/thum_0_".$hoacuoi_anh);
			$sql="DELETE FROM ".TABLE_PREFIX."hoacuoi WHERE md5(hoacuoi_id)='".$edit_id."'";
			execSQL($sql);
