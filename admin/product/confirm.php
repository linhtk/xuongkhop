<?php
	...
	if($action=="Action")
	{
		if(strpos($edit_id,"~~~")===false)
		{
			...
			$sql="SELECT product_image FROM ".TABLE_PREFIX."product WHERE md5(product_id)='$edit_id'";
			$rs=recordset($sql);
			$product_image=$rs['product_image'];
			deletefile("../images/product/thum_0_".$product_image);
			$sql="DELETE FROM ".TABLE_PREFIX."product WHERE md5(product_id)='".$edit_id."'";
			execSQL($sql);
