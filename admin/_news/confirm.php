<?php
	...
	if($action=="Action")
	{
		if(strpos($edit_id,"~~~")===false)
		{
			...
			$sql="SELECT news_image FROM ".TABLE_PREFIX."news WHERE md5(news_id)='$edit_id'";
			$rs=recordset($sql);
			$news_image=$rs['news_image'];
			deletefile("../images/news/thum_0_".$news_image);
			$sql="DELETE FROM ".TABLE_PREFIX."news WHERE md5(news_id)='".$edit_id."'";
			execSQL($sql);
